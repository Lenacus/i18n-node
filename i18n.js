 ...
 * @fork		Forked by Mickaël <m.pannequin@xperteam.fr> on 2017-08-20
 * @link        https://github.com/mashpie/i18n-node
 * @fork-link	https://github.com/Lenacus/i18n-node
...
module.exports = (function() {
	...
	i18n.configure = function i18nConfigure(opt) {
		...
		overwrite = (typeof opt.overwrite === 'string') ? opt.overwrite : '';
		...
		if (Array.isArray(opt.locales) && opt.overwrite) {

			opt.locales.forEach(function(l) {
				read(opt.overwrite + '_' + l);
			});
	...
	i18n.getCatalog = function i18nGetCatalog(object, locale) {
		...
		if (locales[targetLocale]) {
			if (overwrite) {
				var over = locales[overwrite + '_' + targetLocale];
				Object.keys(over).forEach(function(key) {
					locales[targetLocale][key] = over[key];
				});
			}
	...
	var read = function(locale) {
		...
		try {
			fs.readFile(file, 'utf8', function(err, data) {
				if (err) throw err;
				locales[locale] = JSON.parse(data.toString());
			});
		} catch (parseError) {
